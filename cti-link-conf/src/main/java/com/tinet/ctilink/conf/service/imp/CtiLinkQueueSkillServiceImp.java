...
public class CtiLinkQueueSkillServiceImp extends BaseService<QueueSkill> implements CtiLinkQueueSkillService {
    ...
    @Override
    public CtiLinkApiResult<QueueSkill> createQueueSkill(QueueSkill queueSkill) {
        //参数验证
    ...
    @Override
    public CtiLinkApiResult<QueueSkill> deleteQueueSkill(QueueSkill queueSkill) {
        //参数验证
        if (!entityDao.validateEntity(queueSkill.getEnterpriseId())) {
            return new CtiLinkApiResult<>(CtiLinkApiResult.FAIL_RESULT, "参数[enterpriseId]不正确");
        }
    ...
    @Override
    public CtiLinkApiResult<QueueSkill> updateQueueSkill(QueueSkill queueSkill) {
        //参数验证
        if (!entityDao.validateEntity(queueSkill.getEnterpriseId())) {
            return new CtiLinkApiResult<>(CtiLinkApiResult.FAIL_RESULT, "参数[enterpriseId]不正确");
        }
        ...
        return new CtiLinkApiResult<>(dbQueueSkill);
    }

    @Override
    public CtiLinkApiResult<List<QueueSkill>> listQueueSkill(QueueSkill queueSkill) {
        //参数验证
        if (!entityDao.validateEntity(queueSkill.getEnterpriseId())) {
            return new CtiLinkApiResult<>(CtiLinkApiResult.FAIL_RESULT, "参数[enterpriseId]不正确");
        }
        if (queueSkill.getQueueId() == null || queueSkill.getQueueId() <= 0) {
            return new CtiLinkApiResult<>(CtiLinkApiResult.FAIL_RESULT, "参数[queueId]不能为空");
        }
