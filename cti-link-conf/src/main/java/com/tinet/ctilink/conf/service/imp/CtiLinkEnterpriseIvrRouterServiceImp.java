 ...
 */
@Service
public class CtiLinkEnterpriseIvrRouterServiceImp extends BaseService<EnterpriseIvrRouter> implements CtiLinkEnterpriseIvrRouterService {
    ...
    private <T> ApiResult<T> validateEnterpriseIvrRouter(EnterpriseIvrRouter enterpriseIvrRouter){
        ...
        if( ! enterpriseIvrRouter.getRuleAreaProperty().isEmpty()){
            String[] ruleArea = enterpriseIvrRouter.getRuleAreaProperty().trim().split(";");
            Pattern pattern1 = Pattern.compile(Const.AREA_CODE_VALIDATION);
            Pattern pattern2 = Pattern.compile(Const.IS_NUMBER_VALIDATION);


            for(int i=0; i<ruleArea.length; i++){
                Matcher matcher1 = pattern1.matcher(ruleArea[i]);
                Matcher matcher2 = pattern2.matcher(ruleArea[i]);
