...
public class CtiLinkEnterpriseTimeServiceImp extends BaseService<EnterpriseTime> implements CtiLinkEnterpriseTimeService {
    ...
    @Override
    public CtiLinkApiResult<EnterpriseTime> createEnterpriseTime(EnterpriseTime enterpriseTime) {
        ...
        if(enterpriseTime.getName().isEmpty())
            return new CtiLinkApiResult(CtiLinkApiResult.FAIL_RESULT,"时间条件名称不能为空");
        if(enterpriseTime.getType()==null || !(enterpriseTime.getType()==1 || enterpriseTime.getType()==2))
            return new CtiLinkApiResult(CtiLinkApiResult.FAIL_RESULT,"时间类型为1按星期，2按特殊日期");
        if(enterpriseTime.getType()==1){
            enterpriseTime.setFromDay("");
            enterpriseTime.setToDay("");
            if(enterpriseTime.getDayOfWeek().isEmpty())
                return new CtiLinkApiResult(CtiLinkApiResult.FAIL_RESULT,"请选择星期几");
            ...
            for(int i=0;i<dow.length;i++){
                ...
                if(week<1 || week>7)
                    return new CtiLinkApiResult(CtiLinkApiResult.FAIL_RESULT,"星期几对应的取值范围是1-7");
            }
        }
        if(enterpriseTime.getType()==2){
            enterpriseTime.setDayOfWeek("");
            if(enterpriseTime.getFromDay().isEmpty())
                return new CtiLinkApiResult(CtiLinkApiResult.FAIL_RESULT,"起始日期不能为空");
            if(enterpriseTime.getToDay().isEmpty())
                return new CtiLinkApiResult(CtiLinkApiResult.FAIL_RESULT,"截止日期不能为空");
            ...
            try{
                ...
                if(fromDay.compareTo(toDay)>0)
                    return new CtiLinkApiResult(CtiLinkApiResult.FAIL_RESULT,"起始日期不能大于截止日期");
        ...
        if(enterpriseTime.getStartTime().isEmpty())
            return new CtiLinkApiResult(CtiLinkApiResult.FAIL_RESULT,"开始时间不能为空");
        if(enterpriseTime.getEndTime().isEmpty())
            return new CtiLinkApiResult(CtiLinkApiResult.FAIL_RESULT,"结束时间不能为空");
        SimpleDateFormat sdf = new SimpleDateFormat("hh:mm");
        try {
            Date starttime = sdf.parse(enterpriseTime.getStartTime());
            Date endTime = sdf.parse(enterpriseTime.getEndTime());
            if(starttime.compareTo(endTime)>0)
                return new CtiLinkApiResult(CtiLinkApiResult.FAIL_RESULT,"开始时间不能大于结束时间");
        }catch (Exception e){
            e.printStackTrace();
        }
    ...
    @Override
    public CtiLinkApiResult deleteEnterpriseTime(EnterpriseTime enterpriseTime) {
        if(enterpriseTime.getEnterpriseId()==null || enterpriseTime.getEnterpriseId()<=0)
            return new CtiLinkApiResult(CtiLinkApiResult.FAIL_RESULT,"企业编号不能为空");
        if(enterpriseTime.getId()==null || enterpriseTime.getId()<=0)
            return new CtiLinkApiResult(CtiLinkApiResult.FAIL_RESULT,"时间条件id不能为空");
    ...
    @Override
    public CtiLinkApiResult<EnterpriseTime> updateEnterpriseTime(EnterpriseTime enterpriseTime) {
        if(enterpriseTime.getEnterpriseId()==null || enterpriseTime.getEnterpriseId()<=0)
        ...
        if(!(enterpriseTime.getEnterpriseId().equals(et.getEnterpriseId())))
            return new CtiLinkApiResult(CtiLinkApiResult.FAIL_RESULT,"企业编号和时间条件id不匹配");
        if(enterpriseTime.getName().isEmpty())
            return new CtiLinkApiResult(CtiLinkApiResult.FAIL_RESULT,"时间条件名称不能为空");
        ...
        type= et.getType();
        if(type==0){
            return new CtiLinkApiResult(CtiLinkApiResult.FAIL_RESULT,"不存在该id的选项");
        }
        if(type==1){
            enterpriseTime.setFromDay("");
            enterpriseTime.setToDay("");
        ...
        if(type==2){
            ...
            if(enterpriseTime.getFromDay().isEmpty() || enterpriseTime.getToDay().isEmpty()){
                return new CtiLinkApiResult(CtiLinkApiResult.FAIL_RESULT,"请选择起始日期和结束日期");
            }
            try {
                SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
                ...
                if (start.compareTo(end)>0)
                    return new CtiLinkApiResult(CtiLinkApiResult.FAIL_RESULT,"起始日期不能大于结束日期");
            }catch (Exception e) {
                e.printStackTrace();
            }
        }
        ...
        if(enterpriseTime.getStartTime().isEmpty())
            return new CtiLinkApiResult(CtiLinkApiResult.FAIL_RESULT,"请选择开始时间");
        if(enterpriseTime.getEndTime().isEmpty())
            return new CtiLinkApiResult(CtiLinkApiResult.FAIL_RESULT,"请选择结束时间");
        try{
            SimpleDateFormat sdf = new SimpleDateFormat("hh:mm");
            Date startTime = sdf.parse(enterpriseTime.getStartTime());
            Date endTime = sdf.parse(enterpriseTime.getEndTime());
        ...
        if(enterpriseTime.getPriority()==null || enterpriseTime.getPriority()<1)
            return new CtiLinkApiResult(CtiLinkApiResult.FAIL_RESULT,"请选择优先级");
    ...
    @Override
    public CtiLinkApiResult<List<EnterpriseTime>> listEnterpriseTime(EnterpriseTime enterpriseTime) {
        if(enterpriseTime.getEnterpriseId()==null || enterpriseTime.getEnterpriseId()<=0)
            return new CtiLinkApiResult(CtiLinkApiResult.FAIL_RESULT,"企业编号不正确");
        ...
        return new CtiLinkApiResult(enterpriseTimeList);
    }
